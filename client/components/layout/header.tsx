...
export function Header() {
  ...
  return (
    <header className="sticky top-0 z-50 w-full border-b border-border/40 bg-white/80 dark:bg-gray-900/80 backdrop-blur-xl">
          ...
          {!loading && !user ? (
            ...
            <>
              ...
              <Popover>
                ...
                <PopoverContent
                  align="end"
                  className="sm:w-96 w-[calc(100vw-2rem)] max-w-[92vw] max-h-[70vh] p-0 rounded-3xl overflow-hidden shadow-2xl border-border"
                >
                  <div className="p-4 border-b border-border flex items-center justify-between bg-card">
                    ...
                    <Button
                      variant="ghost"
                      size="sm"
                      className="text-xs text-indigo-600 font-bold hover:text-indigo-700 hover:bg-indigo-50 dark:hover:bg-indigo-950/30"
                    >
                      Mark all read
                    </Button>
                  </div>
                    ...
                    <div className="flex flex-col">
                      {notifications.map((n) => (
                        <div
                          key={n.id}
                          className={cn(
                            "p-4 border-b border-border transition-colors hover:bg-muted flex gap-3",
                            n.unread && "bg-indigo-50/30 dark:bg-indigo-950/30",
                          )}
                        >
                          <div className="h-10 w-10 rounded-xl bg-indigo-100 dark:bg-indigo-950 flex items-center justify-center shrink-0">
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <p className="text-xs text-muted-foreground line-clamp-2 leading-relaxed break-words whitespace-normal">{n.description}</p>
                          </div>
                        </div>
                      ))}
                    </div>
                  ...
                  <div className="p-3 text-center border-t border-border">
                    <Button asChild variant="ghost" size="sm" className="w-full text-xs font-bold text-muted-foreground hover:text-primary">
                  ...
                  {!activeChat ? (
                        ...
                        <div className="flex flex-col">
                          {friends.map((friend) => (
                            <button
                              ...
                              className="p-4 border-b border-border transition-colors hover:bg-muted flex gap-3 text-left w-full group"
                            >
                              ...
                              <div className="flex-1 min-w-0">
                                ...
                                <p className="text-xs text-muted-foreground truncate mt-0.5 break-words whitespace-normal">
                    ...
                    <div className="flex flex-col h-[60vh] sm:h-[400px]">
                      <div className="p-3 border-b border-border flex items-center gap-3 bg-card">
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="text-sm font-bold text-foreground truncate">{activeChat.name}</p>
                      ...
                      <ScrollArea className="flex-1 p-4 bg-muted/30">
                        <div className="space-y-4">
                          <div className="bg-card p-3 rounded-2xl rounded-tl-none border border-border shadow-sm max-w-[85%]">
                            <p className="text-xs text-muted-foreground leading-relaxed">
                              Hey! Are you free for a Mock Interview session today?
                            </p>
                            <span className="text-[9px] text-muted-foreground mt-1 block">10:45 AM</span>
                          </div>
                      ...
                      <div className="p-3 bg-card border-t border-border flex gap-2">
                        <input
                          className="flex-1 bg-muted border-0 rounded-xl px-3 text-xs focus:ring-1 focus:ring-indigo-500 outline-none"
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end" className="w-56 p-2 rounded-2xl border-border shadow-xl">
                  <div className="px-2 py-2 mb-2">
                    <p className="text-sm font-bold text-foreground">{user?.fullName || "User"}</p>
      ...
      {/* Mobile Navigation */}
      {mobileMenuOpen && (
        <div className="md:hidden border-t border-border/40 bg-card p-4">
